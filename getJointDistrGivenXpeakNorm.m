...
nBinsY=length(yEdges)-1;
    ...
    for xi=1:nBinsX
        ...
        maxProb=max(currXDistr);
        pxyGivenX(xi,:)=currXDistr/currXDistrTotal;
         %pxyGivenX(xi,:)=currXDistr/maxProb;
    end
    ...
    for xi=1:nBinsX
        currXDistr=pxySmooth(xi,:);
         maxProb=max(currXDistr);
         pxySmooth(xi,:)=currXDistr/maxProb;
    end
        ...
        if(exist('fH','var'))
         omarPcolor(c{1},c{2},pxySmooth',fH)
           ...
           omarPcolor(c{1},c{2},pxySmooth') 
              ...
              cb=colorbar
             ylabel(cb,'Peak-normalized probability given x value')
             colormap(gca,jet)
