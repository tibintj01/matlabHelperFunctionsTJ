...
usePhaseGainPerTime=1;

%usePhaseGainPerTime=0;
...
if(~exist(fullfile(processedDataDir,'allPhaseJumpVsPhaseData.mat'),'file'))
    ...
    for fi=1:totalFieldCount
        ...
        data=load(currFilePath);
                ...
                currPhaseJumpSlope=NaN;
                         ...
                         currStartPhases=allDataByField.startPhasePerCyclePerField{fi};

                         currPhaseGains=allDataByField.phaseGainsPerCyclePerField{fi};
                         
                        
                   
                    if(usePhaseGainPerTime)
                         currPhaseGainsPerTime=currPhaseGains./allDataByField.timeGainsPerCyclePerField{fi};
                    else
                        currPhaseGainsPerTime=currPhaseGains;

                    end
                    
             

                 outOfBoundsIdxes=isnan(currStartPhases) | currPhaseGains>0 | currPhaseGains<-180;
                
                 %outOfBoundsIdxes=isnan(currStartPhases);
                 

                 currStartPhases(outOfBoundsIdxes)=NaN;
                 currPhaseGainsPerTime(outOfBoundsIdxes)=NaN;
                 
                
                 if(length(currPhaseGainsPerTime)<minNumPts)
                     continue
                 end

                 notNaNIdxes=~isnan(currStartPhases) & ~isnan(currPhaseGainsPerTime);
                [phaseGainMovAvgValues] = getBinnedMovingAverages(currStartPhases(notNaNIdxes),currPhaseGainsPerTime(notNaNIdxes),movingAvgStartPhaseWindHalf,movAvgStartPhaseBinCenters);
                 ...
                 %if(~justSummary)
                     ...
                     if(showPlots)
                          ...
                          if(useInvSlope)
                              plot(currStartPhases,1./currPhaseGainsPerTime,'k.','MarkerSize',10)
                          else
                             plot(currStartPhases,currPhaseGainsPerTime,'k.','MarkerSize',10)
                          end
                         
                          
                          %plot(movAvgStartPhaseBinCenters,phaseGainMovAvgValues,'k.','MarkerSize',10)
                ...
                if(useInvSlope)
                    currPhaseGainsPerTime=1./currPhaseGainsPerTime;
                end
                 %rMat=corrcoef([phaseJumpVsTimeVsDistInField(corrIdxes,2),phaseJumpVsTimeVsDistInField(corrIdxes,3)]);
                 
                 
                 notNaNIdxesAvg=~isnan(phaseGainMovAvgValues) & ~isnan(movAvgStartPhaseBinCenters);
                 ...
                 %[m,b,R]=getLinearFit(movAvgStartPhaseBinCenters(notNaNIdxesAvg),phaseGainMovAvgValues(notNaNIdxesAvg));
                 [m,b,R]=getLinearFit(currStartPhases(notNaNIdxes),currPhaseGainsPerTime(notNaNIdxes));
              ...
              masterTimeBinCenters=[masterTimeBinCenters;  movAvgTimeBinCenters(:)];
               ...
               masterPhaseGainsMovAvg=[masterPhaseGainsMovAvg; phaseGainMovAvgValues(:)];
                 ...
                 masterStartPhases=[masterStartPhases;  currStartPhases(:)];
               %masterStartPhaseBinCenters=[masterStartPhaseBinCenters; movAvgStartPhaseBinCenters(:)];
               masterPhaseGains=[masterPhaseGains; currPhaseGainsPerTime(:)];
    ...
    [masterPhaseGainMovAvgValues,masterPhaseGainMovSEMValues] = getBinnedMovingAverages(masterStartPhaseBinCenters,masterPhaseGainsMovAvg,movingAvgStartPhaseWindHalf,movAvgStartPhaseBinCenters);

    save(fullfile(processedDataDir,'allPhaseJumpVsPhaseData.mat'))
else
    load(fullfile(processedDataDir,'allPhaseJumpVsPhaseData.mat'))
...
plot(masterStartPhases,masterPhaseGains,'k.')
%hold on

%plot(movAvgStartPhaseBinCenters,masterPhaseGainMovAvgValues,'k.','LineWidth',5)
hold on
             ...
             %sH=shadedErrorBar(movAvgTimeBinCenters,masterPhaseGainMovAvgValues,masterPhaseGainMovSEMValues,'lineprops',{'r-','linewidth',3})
              ...
              nonNanIdx= ~isnan(masterPhaseGains) & ~isnan(masterStartPhases);
                linCoeffsJump=polyfit(masterStartPhases(nonNanIdx),masterPhaseGains(nonNanIdx),1);
                linModelStartTimes=[0 360];
                linModelPhaseGains=linCoeffsJump(1)*linModelStartTimes+linCoeffsJump(2);
                
                rMat=corrcoef([masterStartPhases(nonNanIdx),masterPhaseGains(nonNanIdx)]);
