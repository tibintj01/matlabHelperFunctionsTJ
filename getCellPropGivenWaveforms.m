function [cellProp] = getCellPropGivenWaveforms(waveforms,spikeTimes,ch,sessionIdx,saveStr,saveDir,alwaysRecompute)
%function [cellProp] = getCellPropGivenWaveforms(subject, szIdx,descriptiveFilename, ch, indWithinCh, nexLoadDir, saveDir)
...
recalculate=0;
if(~exist('alwaysRecompute'))
	recalculate=0;
elseif(alwaysRecompute)
	recalculate=1;
end
...
%ONLY FIRST SEIZURE IN SESSION CURRENTLY
szIdx=1;
indWithinCh=1; %actually all cells on channel
subject=dataInfo(sessionIdx).subject;
descriptiveFilename=dataInfo(sessionIdx).descriptiveFilename;
...
%recalculate every time for development period
%%{
if(recalculate==0)
	if(exist(cellPropSaveFile,'file'))
		disp('unit spike properties already detected, loading.....')
...
numBins = size(waveforms,1); % numBins time points = 1.6 ms
if( numSpikes==0)%Tibin Jan 25, 2019
	cellProp=NaN;
	return
end
...
%% INTEGRATED SPIKE (CUMSUM), width at half max
    ...
    %         spikePChip = interp1(1:numBins,spikeLow,1:splineDt:numBins,'pchip');
    %         figure; hold on;
    %         plot([1:10:numBins*10], spikeLow, 'b');
    %         plot(spikeHigh, 'k');
    %         plot(spikePChip, 'r');
    %     end
end
%toc;
% IMPORTANT - convert spikeMaxIndex into low resolution (1-numBins) bin counts.
...
%save('testSaveMat.mat','spikeT01Width')
	...
	save(cellPropSaveFile, ...
	    ...
	    'meanRate','waveforms','saveStr','sessionIdx');
%fds
