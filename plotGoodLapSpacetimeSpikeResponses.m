...
if(~exist('dmasterSpacetimePhaseResponse.mat','file'))
...
for fi=startFileID:length(filePaths)
    ...
    if(justAchilles)
        if(contains(currFilePath,'chilles'))
            continue
        end
    end
    ...
    for di=1:2
        ...
        if(di==2)
            ...
            if(~isfield(data,'leftSpikesRefPhases'))
                  continue
              end
            
             unitDirSpikeTimes=data.leftSpikeTimes;
            unitDirSpikeRefPhases=data.leftSpikesRefPhases;
            unitDirSpikeRefCycleIDs=data.leftSpikeRefCycleIDs;
        ...
        elseif(di==1)
            ...
            unitDirSpikeRefPhases=data.rightSpikesRefPhases;
            unitDirSpikeRefCycleIDs=data.rightSpikeRefCycleIDs;
        end
        ...
        if(max(localGaussSpikeRatePerPosTime)==0)
       ...
       if(autoShiftOffset)
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         minSpeed=0.05;
    lowSpeedIdxes=abs(data.speedPerTimeStep(:))<minSpeed;
       ...
       if(isempty(avgFieldSpeedPerLap))
        ...
        for fii=1:currNumFields
            ...
            for li=1:currNumLaps
                ...
                timeIdxesInLap=data.positionTimeAxis>=lapStartTimes(li) & data.positionTimeAxis<=lapStopTimes(li);
                ...
                currFieldLapTimes=refUnitData.positionTimeAxis(allTimeIdxesInCurrFieldAndLap);
                currSessionExpStartTime=refUnitData.positionTimeAxis(1);
                ...
                expectedFieldEnd=data.(sprintf('manualFieldEndsM%s',currFieldDirection))(fii);
                expectedFieldStart=data.(sprintf('manualFieldStartsM%s',currFieldDirection))(fii);
                ...
                avgFieldSpeedPerLap(fii,li)=nanmean(abs(data.speedPerTimeStep(allTimeIdxesInCurrFieldAndLap)));
                
                hasMinSeqSpeedIdxesPerTime=abs(data.speedPerTimeStep(:)')>minSeqSpeed;
                
                 if(length(fieldNumPerTime) ~= length(data.speedPerTimeStep(:)))
                    continue
                 end
        ...
        daspect([1 360*1.5 1])
        saveas(gcf,sprintf('%s_examplePhaseVsTimeSingleCell_%s.tif',fileBaseName,currFieldDirection))
    ...
    %masterSpeedPerTime=[masterSpeedPerTime; abs(data.speedPerTimeStep(:))];
        masterSpeedPerTime=[masterSpeedPerTime; data.absOneDspeedPerTimeStep(:)];
        ...
        masterDirectionPerTime=[masterDirectionPerTime; repmat(di,size(localSpikePhasePerTime(:)))];
         ...
         plotSpacetimePhaseHeatMap([localNormDistInFieldPerTime(:), localNormTimeInFieldPerTime(:), localSpikePhasePerTime(:)],heatResUnit,0,[],localGaussSpikeRatePerPosTime(:));
        ...
        saveas(gcf,sprintf('%s_SpaceTimeResponses_%s.tif',fileBaseName,currFieldDirection))
...
[latestOutputDataStruct]=plotSpacetimePhaseHeatMap([masterDistPerTime(:), masterLapTimePerTime(:), masterSpikePhasePerTime(:)],heatRes,1,[],masterSpikeRatePerTime(:));
   ...
   % getLinearXYCoeffsforZstrata(x,y,z)
    ...
    %scatter(localDistInFieldPerTime,localTimeInFieldPerTime,20,localSpikePhasePerTime,'filled')
        ...
        plot(xlim,[1 1],'k--','LineWidth',3)
         ...
         daspect([1 1 1])
         maxFig
         saveas(gcf,sprintf('HC11_SummaryAvgSpaceTimeResponse.tif'))
         %%
         timeFig=figure(111); 
         distFig=figure(1111); 
