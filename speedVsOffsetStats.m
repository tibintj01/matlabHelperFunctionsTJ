...
if(~exist('dspeedVsOffsetStatsOct30_2021.mat','file'))
tic
...
%load('speedVsOffsetData_HighResSpeed_Oct25.mat')
load('speedVsOffsetData_HighResSpeed_Nov5.mat')
close all
toc
...
%speedGroupColors=magma(numSpeedGroups);
...
for si=compareGps
  ...
  currSpeedGroupPhaseVsTime=allPhaseVsTimePerSpeedGroupAllFields{si};
    ...
    lateFieldIdxes=currSpeedGroupPhaseVsTime(:,1)>maxTime;


    %currSpeedGroupPhaseVsTime(lateFieldIdxes,:)=NaN;
  ...
  [timeBinCenters,currSpeedGpPhaseAvgValues] = getBinnedCircAverages(currSpeedGroupPhaseVsTime(:,1)/trueTimeBoundFact,currSpeedGroupPhaseVsTime(:,2),timeEdgesAvg);
    currSpeedGpPhaseAvgValues=mod(currSpeedGpPhaseAvgValues,360);
  ...
  %currSpeedGpPhaseAvgValues(timeBinCenters>maxTime)=NaN;
...
subplot(1,3,2)
plot(localAvgSpeedBinCenters/(localAvgSpeedBinCenters(2)), (localAvgOffsetPerSpeed-(localAvgOffsetPerSpeed(2)))/360,'k-','LineWidth',3)
...
plot(localAvgSpeedBinCenters/(localAvgSpeedBinCenters(2)), (localAvgOffsetPerSpeed-(localAvgOffsetPerSpeed(2)))/360,'k.','MarkerSize',50)

dataSpeedRatioCenters=localAvgSpeedBinCenters/(localAvgSpeedBinCenters(2));
dataOffsetCycleFracs=(localAvgOffsetPerSpeed-(localAvgOffsetPerSpeed(2)))/360;

save('speedVsOffsetOct25.mat','dataSpeedRatioCenters','dataOffsetCycleFracs')
...
for bi=1:length(testBases)
    ...
    %subplot(4,5,bi)
     ...
     plot(localAvgSpeedBinCenters, localAvgOffsetPerSpeed,'k.','MarkerSize',50)
          ...
          hold on
          plot(localAvgSpeedBinCenters, localAvgOffsetPerSpeed,'k-','LineWidth',4)
          logSpeed=scaledata(getLogTime(t+timeOffset,base),minOffset,maxOffset);
...
ylabel('Phase precession offset (deg)')
 ...
 %subplot(1,3,3)
 figure
 %localAvgSlopePerSpeed=mod(localAvgSlopePerSpeed,360);
...
ylabel('Phase precession slope (deg/field)')
 ...
 %ylim([0 1])
    ...
    save('speedVsOffsetStatsOct30_2021.mat')
else
    
    load('speedVsOffsetStatsOct30_2021.mat')
end
%%
...
lowSpeedCategoryIdx=1:3;
highSpeedCategoryIdx=6:8;

lowSpeedCategoryIdx=2:4;
highSpeedCategoryIdx=6:8;
...
lowSpeedPhaseVsTime=vertcat(allPhaseVsTimePerSpeedGroupAllFields{lowSpeedCategoryIdx});
highSpeedPhaseVsTime=vertcat(allPhaseVsTimePerSpeedGroupAllFields{highSpeedCategoryIdx});

maxTime=1;

%
lateFieldIdxesLowSpeed=lowSpeedPhaseVsTime(:,1)>maxTime;
lateFieldIdxesHighSpeed=highSpeedPhaseVsTime(:,1)>maxTime;

lowSpeedPhaseVsTime(lateFieldIdxesLowSpeed,:)=NaN;
highSpeedPhaseVsTime(lateFieldIdxesHighSpeed,:)=NaN;
...
lowSpeedOffset=nanmean(localAvgOffsetPerSpeed(lowSpeedCategoryIdx));
highSpeedOffset=nanmean(localAvgOffsetPerSpeed(highSpeedCategoryIdx));
lowSpeedSlope=nanmean(localAvgSlopePerSpeed(lowSpeedCategoryIdx));
highSpeedSlope=nanmean(localAvgSlopePerSpeed(highSpeedCategoryIdx));
...
offsetDiffsEdges=linspace(-40,40,100);
offsetDiffsEdges=linspace(-60,60,250);
%slopeRatiosEdges=linspace(0.8,1.25,50);

slopeRatiosEdges=linspace(0.5,1.5,70);
...
pValOffset=1-sum(No(originalOffsetHighMinusLow>offsetDiffBins))/sum(No);

box off
xlabel('High speed to low speed offset difference (deg)')
ylabel('Shuffle count')
subplot(2,1,2); 
Ns=histcounts(shuffledSlopesRatios,slopeRatiosEdges); 

pValSlope=1-sum(Ns(originalSlopeHighMinusLow>slopeRatioBins))/sum(Ns);
