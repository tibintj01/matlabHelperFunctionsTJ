function [] = concatenateLFPchanAcrossMatFiles(sessNum,dataDir)
	...
	subject=dataInfo(sessNum).subject;
	 ...
	 for chan=1:numChannels
		 ...
		 if(~exist(fullfile(dataDir,sprintf('concatChan%d',chan))))
		 	mkdir(fullfile(dataDir,sprintf('concatChan%d',chan)))
		 else
		 	delete(fullfile(dataDir,sprintf('concatChan%d',chan),'*.mat'));
		 end

		save((fullfile(dataDir,sprintf('concatChan%d',chan),'lfpOriginalConcat.mat')),'concatenatedLFP','-v7.3')
