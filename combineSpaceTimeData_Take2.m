...
plotIndividualPhasePrecession=1;
...
for i=1:numFilesToUse
    ...
    phaseVsTime=currData.spaceTimePhaseInfo.avgPhasePerTimeBin;
    phaseVsTimeCov=corrcoef(timeVals,phaseVsTime);
    
    
    phaseVsSpaceR=phaseVsSpaceCov(1,2);
    %if(trackDir==2)
     if(trackDir==1)
     %if((phaseVsSpaceR)<minSpaceCorr || phaseVsSpaceR>maxSpaceCorr)
         if((phaseVsSpaceR)<minSpaceCorr || phaseVsSpaceR>maxSpaceCorr)
         %if((phaseVsSpaceR)>-minSpaceCorr)
          %if((phaseVsSpaceR)<-minSpaceCorr)
   ...
   if(length(spaceBins)<minNumSpaceBinsIndiv || length(timeBins)<minNumTimeBinsIndiv || abs(estSpeed(numCellsUsed+1))<minSpeed)
     estSpeed(numCellsUsed+1)=NaN;
       continue 
   end
   ...
   if(plotIndividualPhasePrecession)
       plot(currFieldSpaceTimePhaseTriplets(:,1),currFieldSpaceTimePhaseTriplets(:,3),'k.')
       drawnow
...
save('allFieldSpaceTimePhaseTriplets.mat','allFieldSpaceTimePhaseTriplets')
