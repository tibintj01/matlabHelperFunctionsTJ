    ...
    for di=1:2
        ...
        for unit1Num=1:numUnits
            ...
            %for unit2Num=(unit1Num+1):numUnits %vs upper triangle matrix
                ...
                if(alreadyProcessed(unit1Num,unit2Num) || unit1Num==unit2Num)
                    continue
                end
                ...
                for ci=1:numCycles
                    ...
                    if(~isnan(timeDifferencesPerCycle(unit1Num,unit2Num,ci)))
                        if(abs(timeDifferencesPerCycle(unit1Num,unit2Num,ci))<cycleDur*lastFrac)
       ...
       timeOfCycleTrough=currThetaSeqData.timeOfCycleTrough;
        ...
        for unit1Num=1:numUnits
            for unit2Num=1:numUnits %vs upper triangle matrix  
                ...
                if(unit1Num==unit2Num)
                    continue
                end
                %not overlapping
                %if(numCyclesCofiring(unit1Num,unit2Num,di) < numCyclesThreshPerRow(unit1Num))
                if(numCyclesCofiring(unit1Num,unit2Num,di) < ABSOLUTE_THRESH_NUM_CYCLES_COFIRING)
                    continue
                end
                ...
                for ci=1:numCycles
                    ...
                    if(unit1Num~=unit2Num && unit1TimingCurrCycle > lateCycleTimeThresh && unit2TimingCurrCycle >lateCycleTimeThresh)   
                        if(restrictToCorrectOrderCycles)
                            if(unit1TimingCurrCycle<unit2TimingCurrCycle)
       ...
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        for ui=1:numUnits
            for uii=(ui+1):numUnits
                ...
                unit1PhasePerCycle=phasePerUnitPerCycle(ui,:);
                unit2PhasePerCycle=phasePerUnitPerCycle(uii,:);
