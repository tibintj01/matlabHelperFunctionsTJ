...
if(~exist('phaseVsTimeAndLogTimeStats.mat','file'))
    ...
    totalFieldCount=logStatsInfo.totalFieldCount;
    for fi=1:totalFieldCount
        if(fieldIsUsedForLogTimeStats(fi))
            allFieldSpikePhases=[allFieldSpikePhases; data.spikePhasesPerField{fi}];
            spikeTimeFracInFieldPerField=[spikeTimeFracInFieldPerField; data.spikeTimeFracInFieldPerField{fi}];
    ...
    showPlots=1;
        ...
        allFieldToFieldEndLogTimes=getLogTime(1-spikeTimeFracInFieldPerField-timeOffset,base);
               % allFieldToFieldEndLogTimes=getLogTime(1-spikeTimeFracInFieldPerField,base);

        %allFieldToFieldEndLogTimes=getLogTime(spikeTimeFracInFieldPerField);
        ...
        if(showPlots)
            ...
            xlabel(sprintf('log_{%.1f}(time in field)',base))
        ...
        if(showPlots)
            ...
            getJointDistr(1-allFieldToFieldEndLogTimes+normrnd(0,timeNoiseSigma,size(allFieldToFieldEndLogTimes)),allFieldSpikePhases,logTimeFieldEdges,phaseEdges,fH,withSmooth);

            %+normrnd(0,0.02,size(allFieldToFieldEndLogTimes)
            %xlabel('log(time to end of field) (normalized)')
            
              xlabel(sprintf('log_{%.1f}(1-time in field)',base))
              ylabel('Theta phase (deg)')
           ...
           [~,bestShift]=min(abs(shiftErrors));
            ...
            getJointDistr(allFieldToFieldEndTimes+normrnd(0,timeNoiseSigma,size(allFieldToFieldEndTimes)),allFieldSpikePhases,timeFieldEdges,phaseEdges,fH,withSmooth);
