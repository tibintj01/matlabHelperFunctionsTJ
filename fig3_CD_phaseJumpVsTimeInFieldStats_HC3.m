    ...
    for di=1:2
        ...
        elseif(di==1)
           ...
           if(isnan(phaseJumpVsTimeVsDistInField))
             ...
             if(length(currStartPhasesInField)<minNumPts|| length(currPhaseGains)<minNumPts)
                 continue
             end
           ...
           masterAvgPhaseGains=[masterAvgPhaseGains; phaseGainMovAvgValues(:)];
           
           masterPhaseGains=[masterPhaseGains; currPhaseGains(:)];
           masterStartPhases=[masterStartPhases; currStartPhasesInField(:)];
...
[masterPhaseGainMovAvgValues,masterPhaseGainMovSEMValues] = getBinnedMovingAverages(masterStartPhaseBinCenters,masterAvgPhaseGains,movingAvgStartPhaseWindHalf,movAvgStartPhaseBinCenters);
...
%hold on
             ...
             %sH=shadedErrorBar(movAvgTimeBinCenters,masterPhaseGainMovAvgValues,masterPhaseGainMovSEMValues,'lineprops',{'r-','linewidth',3})
              ...
              nonNanIdx=~isnan(masterStartPhaseBinCenters) & ~isnan(masterAvgPhaseGains) & masterStartPhaseBinCenters>=startRegressionPhase &  masterStartPhaseBinCenters<=endRegressionPhase;
                linCoeffsJump=polyfit(masterStartPhaseBinCenters(nonNanIdx),masterAvgPhaseGains(nonNanIdx),1);
                linModelStartPhases=[0 360];
                linModelPhaseGains=linCoeffsJump(1)*linModelStartPhases+linCoeffsJump(2);
                
                rMat=corrcoef([masterStartPhaseBinCenters(nonNanIdx),masterAvgPhaseGains(nonNanIdx)]);
           ...
           lgd.FontSize=12;
             ...
             saveas(gcf,fullfile(figurePanelDir,'fig3C_GainVsPhase_HC3.tif'))
             print(fullfile(figurePanelDir,'fig3C_GainVsPhase_HC3'),'-depsc')
...
uberTitle(sprintf('HC3 phase gain vs phase correlations (min speed: %.2f m/s)',minSeqSpeed))

saveas(gcf,fullfile(figurePanelDir,'fig3D_GainVsPhaseDistr_HC3.tif'))
             print(fullfile(figurePanelDir,'fig3D_GainVsPhaseDistr_HC3'),'-depsc')
