function [] = analyticalAbstraction(speedFact)

assemblyN=50;
normDIall=NaN(assemblyN,1);
rawDIall=NaN(assemblyN,1);

%for noiseConditionID=1:assemblyN
for noiseConditionID=1:10
...
filePath=sprintf('../processedData/seqDrivenPlaceNetworkMeasurements%.4f_N_%d.mat',speedFact,N);
...
tic
	...
	if(rerun || ~exist(filePath,'file'))
		...
		while(~allCellsReached)
			...
			%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			%winner takes all circuit spiking mechanism; most activated population sends dendritic potentials
			%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		if(detectAssemblies)
			...
			save(filePath,'W','H','A','speedFact','connectivityStr','functionalConnectivityMovie','dendriticActivationOverTime','numCycles')
		else
			if(plotDendriticDrive)
				...
				if(noiseConditionID==1 || noiseConditionID==assemblyN)
...
%{
%pairwiseFactorSimilarity=W'*W; %17xN * Nx17=17x17
pairwiseFactorSimilarity=cov(Wnorm);
for i=1:size(pairwiseFactorSimilarity,1)
	%pairwiseFactorSimilarity(:,i)=pairwiseFactorSimilarity(:,i)/norm(pairwiseFactorSimilarity(:,i));
	pairwiseFactorSimilarity(i,i)=NaN;
end

figure
imagesc(pairwiseFactorSimilarity)
colorbar
%caxis([0 0.5])
%}
%{
...
for i=1:cycleID
	...
	assemblyPatternMatrix=W(:,1:numFactorsUsed);%Nx17
