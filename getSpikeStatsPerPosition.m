function [positionBins,firingRatePerPositionRight, firingRatePerPositionLeft,spikeTimeDirectionAssignment] = getSpikeStatsPerPosition(spikeTimes,positionTimeAxis,positionPerTimeStep,speedPerTimeStep)
%{
...
positionPerTimeStepRight(speedPerTimeStep<0.05)=NaN; %velocity was at least 5 cm/s

positionPerTimeStepLeft=positionPerTimeStep;
%positionPerTimeStepLeft(speedPerTimeStep>0)=NaN;
positionPerTimeStepLeft(speedPerTimeStep>-0.05)=NaN; %s velocity was at least 5 cm/s
...
spikeTimeDirectionAssignment=NaN(size(spikeTimes));
...
for direction=1:2
    ...
    for ti=1:length(positionTimeAxis)
        ...
        spikeTimeDirectionAssignment(spikeTimes>=binStartTime & spikeTimes<binEndTime)=direction;
        
        %spikeTimes>=binStartTime & spikeTimes<binEndTime & 
